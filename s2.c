// Distributed File System - S2 Server Implementation (PDF Files)
...
int main() {
    ...
    printf("S2 (PDF) server listening on port %d\n", PORT);
    ...
    while (1) {
        ...
        printf("Receiving PDF file: %s\n", filename);
        
        // Receive file data
        char file_data[BUFFER_SIZE * 10] = {0}; // Larger buffer for file data
        int bytes_received = read(new_socket, file_data, sizeof(file_data));
        ...
        printf("PDF file %s stored successfully\n", filename);
        close(new_socket);
    }
